...
string * find(string in)
	{
		...
		string * poss; //something suggests i perhaps ought to move this declaration to somewhere 'global', to avoid creating memory leaks and such...
...
int main (int argc, char* argv[])
{
	...
	if (bevorse == true) {
		cout<<TITLE<<" v"<<VERSION<<"-"<<CHNG<<" written on "<<WRITTEN<<" by "<<ACKNOWLEDGE<<" (last update: "<<UPDATED<<")\n\tCompiled for locale ["<<LANG<<"] at "<<__TIME__<<" on "<<__DATE__<< endl;
						...
						for(int u = 0; u < cd; u++)
						{
							...
							for(int nl = 0; nl < lu; nl++)
							{
								for(int nr = 0; nr < ru; nr++)
								{
									...
									{
										cout<<a<<"\t"<<azPoss[u].substr(0,4)<<"\t"<<left[nl]<<"\t"<<right[nr]<<"\t"<<azPoss[u].substr(4,4)<<"\t"<<z;
if(bevorse){cout<<"\t\t\t\t\t[WORKING]"<< endl;}
